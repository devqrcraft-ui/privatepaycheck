...
const BS = [{min:0,max:12400,rate:0.10},{min:12400,max:50400,rate:0.12},{min:50400,max:105700,rate:0.22},{min:105700,max:201775,rate:0.24},{min:201775,max:256225,rate:0.32},{min:256225,max:640600,rate:0.35},{min:640600,max:1e9,rate:0.37}];
const BM = [{min:0,max:24800,rate:0.10},{min:24800,max:100800,rate:0.12},{min:100800,max:211400,rate:0.22},{min:211400,max:403550,rate:0.24},{min:403550,max:512450,rate:0.32},{min:512450,max:768700,rate:0.35},{min:768700,max:1e9,rate:0.37}];
...
export default function TwoJobsClient() {
  ...
  const sel=(label:string,val:string,set:(v:string)=>void,opts:[string,string][])=>(
    <div key={label}>
      ...
      <select value={val} onChange={e=>set(e.target.value)} style={{width:'100%',background:'#1e2a4a',border:'1.5px solid rgba(255,255,255,0.12)',borderRadius:'10px',padding:'11px 14px',color:'#fff',fontSize:'15px',fontFamily:'inherit',outline:'none',boxSizing:'border-box' as const, colorScheme:'dark'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(180px,1fr))',gap:'16px',marginBottom:'20px'}}>
            {[['Job 1 Annual Salary ($)',j1,setJ1,'60000'],['Job 2 Annual Salary ($)',j2,setJ2,'25000']].map(([label,val,set,ph])=>(
              <div key={label as string}>
                ...
                <input type="number" value={val as string} onChange={e=>(set as (v:string)=>void)(Math.max(0,+e.target.value).toString())} min={0} placeholder={ph as string} style={{width:'100%',background:'#1e2a4a',border:'1.5px solid rgba(255,255,255,0.12)',borderRadius:'10px',padding:'11px 14px',color:'#fff',fontSize:'15px',fontFamily:'inherit',outline:'none',boxSizing:'border-box' as const}}/>
              </div>
            ))}
            ...
            <div>
              ...
              <select value={state} onChange={e=>setState(e.target.value)} style={{width:'100%',background:'#1e2a4a',border:'1.5px solid rgba(255,255,255,0.12)',borderRadius:'10px',padding:'11px 14px',color:'#fff',fontSize:'15px',fontFamily:'inherit',outline:'none',boxSizing:'border-box' as const, colorScheme:'dark'}}>
