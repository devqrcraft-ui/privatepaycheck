          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(180px,1fr))',gap:'16px',marginBottom:'20px'}}>
            {[['Job 1 Annual Salary ($)',j1,setJ1,'60000'],['Job 2 Annual Salary ($)',j2,setJ2,'25000']].map(([label,val,set,ph])=>(
              <div key={label as string}>
                ...
                <input type="number" value={val as string} onChange={e=>(set as (v:string)=>void)(e.target.value)} min={0} placeholder={ph as string} style={{width:'100%',background:'#1e2a4a',border:'1.5px solid rgba(255,255,255,0.12)',borderRadius:'10px',padding:'11px 14px',color:'#fff',fontSize:'15px',fontFamily:'inherit',outline:'none',boxSizing:'border-box' as const}}/>
              </div>
            ))}
