...
export default function W4Client(){
      ...
      <div style={{maxWidth:'900px',margin:'0 auto',padding:'32px 16px'}}>
        ...
        <div style={{background:'rgba(255,255,255,0.04)',border:'1px solid rgba(201,168,76,0.3)',borderRadius:'16px',padding:'28px',marginBottom:'24px'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(170px,1fr))',gap:'16px',marginBottom:'16px'}}>
            <div><label style={lbl}>Annual Salary ($)</label><input type="number" value={salary} onChange={e=>setSalary(e.target.value)} min={1} style={inp}/></div>
            <div>
              <label style={lbl}>Filing Status</label>
              <select value={filing} onChange={e=>setFiling(e.target.value)} style={{...inp, colorScheme:'dark', background:'#1e2a4a', color:'#fff'}}>
                <option value="single">Single</option>
                <option value="married">Married Filing Jointly</option>
              </select>
            </div>
            <div><label style={lbl}>401(k) Contribution/yr</label><input type="number" value={k401} onChange={e=>setK401(e.target.value)} min={0} style={inp} placeholder="0"/></div>
            <div><label style={lbl}>Other Income/yr (freelance, etc)</label><input type="number" value={otherIncome} onChange={e=>setOtherIncome(e.target.value)} min={0} style={inp} placeholder="0"/></div>
            <div><label style={lbl}>Itemized Deductions/yr</label><input type="number" value={deductions} onChange={e=>setDeductions(e.target.value)} min={0} style={inp} placeholder="0"/></div>
            <div><label style={lbl}>Tax Credits/yr (child, etc)</label><input type="number" value={credits} onChange={e=>setCredits(e.target.value)} min={0} style={inp} placeholder="0"/></div>
          </div>
