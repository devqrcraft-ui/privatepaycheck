...
const STATE_DATA: Record<string, { name: string; maxWeekly: number; maxWeeks: number; rate: number; waitWeek: boolean; stateIncomeTax: number }> = {
  ...
  "illinois": { name: "Illinois", maxWeekly: 742, maxWeeks: 26, rate: 47, waitWeek: true, stateIncomeTax: 4.95 },
  ...
  "new-hampshire": { name: "New Hampshire", maxWeekly: 427, maxWeeks: 26, rate: 50, waitWeek: false, stateIncomeTax: 0 },
  ...
  "texas": { name: "Texas", maxWeekly: 563, maxWeeks: 26, rate: 47, waitWeek: false, stateIncomeTax: 0 },
...
export default async function Page({ params }: Props) {
 ...
 + maxWeekly + ' per week for up to ' + maxWeeks + ' weeks — a total of up to 
import { notFound } from 'next/navigation';
import UnemploymentCalculatorState from './UnemploymentCalculatorState';
...
export default async function Page({ params }: Props) {
 ...
 + maxTotal.toLocaleString() + '. Your actual benefit depends on your prior wages.' } },
    { '@type': 'Question', name: 'How is unemployment calculated in ' + stateName + '?', acceptedAnswer: { '@type': 'Answer', text: stateName + ' calculates unemployment benefits based on your highest-earning base period quarter. The weekly benefit is approximately ' + rate + '% of your average weekly wage, up to the 
import { notFound } from 'next/navigation';
import UnemploymentCalculatorState from './UnemploymentCalculatorState';
...
export default async function Page({ params }: Props) {
 ...
 + maxWeekly + ' maximum.' } },
    { '@type': 'Question', name: 'How long can I collect unemployment in ' + stateName + '?', acceptedAnswer: { '@type': 'Answer', text: 'Standard unemployment benefits in ' + stateName + ' last up to ' + maxWeeks + ' weeks. During periods of high unemployment, extended benefits may be available.' } }
  ]};
}
