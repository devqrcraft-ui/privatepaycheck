...
# 2. Знайти layout і додати Footer
LAYOUT=$(find src -name "layout.tsx" | head -1)
echo "Layout: $LAYOUT"
sed -i "3a import Footer from '@/components/Footer';" $LAYOUT
sed -i 's|</body>|<Footer />\n      </body>|' $LAYOUT
grep -n "Footer\|</body>" $LAYOUT

# 3. Пуш
git add -A && git commit -m "feat: add Footer with internal links for SEO indexing" && git push